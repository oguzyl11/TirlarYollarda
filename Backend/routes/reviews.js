...
router.post('/', auth, [
    ...
    try {
        ...
        if (!jobData) {
            return res.status(400).json({
                success: false,
                message: 'İş bulunamadı. Lütfen geçerli bir iş seçin.'
            });
        }
        if (jobData.status !== 'completed') {
            return res.status(400).json({
                success: false,
                message: 'Sadece tamamlanan i�ler i�in yorum yap�labilir'
            });
        }
        ...
        if (existingReview) {
            return res.status(400).json({
                success: false,
                message: 'Bu i� i�in zaten yorum yapt�n�z'
            });
        }
        ...
        if (reviewee === req.user.userId) {
            return res.status(400).json({
                success: false,
                message: 'Kendinize yorum yapamazs�n�z'
            });
        }
        ...
        res.status(201).json({
            success: true,
            message: 'Yorum ba�ar�yla olu�turuldu',
        ...
        res.status(500).json({
            success: false,
            message: 'Yorum olu�turulurken hata olu�tu'
        });
    }
});
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            message: 'Yorumlar getirilirken hata olu�tu'
        });
    }
});
